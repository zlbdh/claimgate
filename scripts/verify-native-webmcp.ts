...
async function main() {
  ...
  try {
    ...
    database.close();
    const inspected = JSON.stringify({ rawResults, html, activity, clientLogs, serverLogs });
    ...
    console.log(JSON.stringify({
      ...
      scans: "raw results, HTML, activity, browser/server logs contained no real internal inventory ID",
    }, null, 2));
  } finally {
    await browser?.close();
