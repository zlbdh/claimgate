...
import { createEvidenceDigester, EVIDENCE_SLOTS } from "@/features/evidence/evidence-digester";
import { createKeyring } from "@/server/security/keyring";
import { initializeDatabase } from "./migrate";
import {
  privateEvidenceAppearsInForTest,
  privateEvidenceSeedsAreDistinctForTest,
  verifyFictionalSeedForTest,
} from "./private-evidence-seed";
...
function clientBuildFiles(): Buffer[] {
  const root = resolve(".next/static");
  if (!existsSync(root)) return [];
  return readdirSync(root, { recursive: true, withFileTypes: true })
    .filter((entry) => entry.isFile())
    .map((entry) => readFileSync(resolve(entry.parentPath, entry.name)));
}
...
describe("server-internal evidence repository", () => {
  ...
  it("同一主密钥重启后验证强候选；换钥在 repository/service 前失败", () => {
    ...
    expect(verifyFictionalSeedForTest({
      seedIndex: 0,
  ...
  it("私有答案彼此不同，且不跨 DB/DTO/audit/log/serialization/client bundle 边界", () => {
    ...
    expect(privateEvidenceSeedsAreDistinctForTest()).toBe(true);
    expect(publicValues.map((value) => privateEvidenceAppearsInForTest(value)))
