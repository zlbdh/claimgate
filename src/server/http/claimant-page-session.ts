...
export function readClaimantPageSession(
  ...
  try {
    ...
    if (session.role !== "CLAIMANT") return null;
...
export function mintReportCsrf(input: {
  runtime: HttpRuntime;
  session: DemoSession;
  ...
  if (route.method !== "POST" || route.action === null || route.requiresOneTime !== input.oneTime) {
    throw new DomainError("CONFIGURATION_ERROR");
  }
  const resolved = resolveAuthenticatedRoute(new Request(`${input.runtime.appOrigin.origin}${input.path}`, {
    method: "POST",
  }), input.routeKey);
