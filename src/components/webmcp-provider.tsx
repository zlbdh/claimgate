...
export function WebMcpProvider({ children }: { children: ReactNode }) {
  ...
  const [registration, setRegistration] = useState<PageRegistration>();
  ...
  const unmount = useCallback((id: symbol) => {
    setCandidates(undefined);
    setRegistration((current) => current?.id === id ? undefined : current);
  }, []);
  ...
  }, [activity, effectiveScope, publishCandidates, registration, router]);
