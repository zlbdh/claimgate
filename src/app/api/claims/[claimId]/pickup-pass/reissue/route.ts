...
import { parsePickupIssuanceForm } from "@/features/claims/pickup-pass-form";
import {
...
export function createReissuePickupPassRouteHandler(dependencies: ReportRouteDependencies) {
  return async function reissuePickupPass(request: Request): Promise<Response> {
    try {
      const preflight = preflightAuthenticatedRequest({
        ...
        repository: dependencies.repository, routeKey: "api.claims.pickup.reissue",
      });
      const command = parsePickupIssuanceForm(await readStrictUrlEncodedForm(request));
      ...
      const result = executeAuthorizedMutation({
        ...
        }).reissue(reportActorContext(context), preflight.params.claimId!, command),
      });
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return createReissuePickupPassRouteHandler(getHttpRuntime())(request);
  } catch (error) { return sensitiveApiError(error); }
}
