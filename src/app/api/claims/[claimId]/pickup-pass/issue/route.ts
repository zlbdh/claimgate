...
export function createIssuePickupPassRouteHandler(dependencies: ReportRouteDependencies) {
  return async function issuePickupPass(request: Request): Promise<Response> {
    try {
      const preflight = preflightAuthenticatedRequest({
        ...
        repository: dependencies.repository, routeKey: "api.claims.pickup.issue",
      });
      ...
      const result = executeAuthorizedMutation({
        ...
        }).issue(reportActorContext(context), preflight.params.claimId!, command),
      });
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return createIssuePickupPassRouteHandler(getHttpRuntime())(request);
  } catch (error) { return sensitiveApiError(error); }
}
