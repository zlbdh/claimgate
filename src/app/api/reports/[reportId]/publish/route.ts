...
export function createPublishReportRouteHandler(dependencies: ReportRouteDependencies) {
  return async function publishReport(request: Request): Promise<Response> {
    try {
      const preflight = preflightAuthenticatedRequest({
        ...
        repository: dependencies.repository, routeKey: "api.reports.publish",
      });
      ...
      executeAuthorizedMutation({
        ...
        mutation: (repository) => reportService(dependencies, repository)
          .publish(reportActorContext(context), reportId, form.expectedVersion),
      });
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return createPublishReportRouteHandler(getHttpRuntime())(request);
  } catch (error) {
    return mapApiError(error);
  }
}
