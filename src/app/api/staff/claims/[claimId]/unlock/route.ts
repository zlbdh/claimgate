...
export function createUnlockClaimRouteHandler(dependencies: ReportRouteDependencies) {
  return async function unlockClaim(request: Request): Promise<Response> {
    try {
      const preflight = preflightAuthenticatedRequest({
        ...
        repository: dependencies.repository, routeKey: "api.staff.claims.unlock",
      });
      ...
      const result = executeAuthorizedMutation({
        ...
        }).unlock(reportActorContext(context), preflight.params.claimId!, command),
      });
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return createUnlockClaimRouteHandler(getHttpRuntime())(request);
  } catch (error) { return mapApiError(error); }
}
